...
#endif
{
    ...
    void link_signals(const char *module_path, const char *sim_domain)
    {
		...
		int o_avg_busy_is_clock = esc_link_clockgen( &o_avg_busy, sc_time( 0, SC_NS ), module_path, sim_domain, "o_avg_busy" );
		...
		esc_link_signals( &o_avg_busy, module_path, sim_domain, true, inputDelay );
		esc_link_signals( &o_avg_vld, module_path, sim_domain, true );
		esc_link_signals( &o_avg_data, module_path, sim_domain, true );
		...
		if ( o_avg_busy_is_clock )
		{
			sc_signal_in_if< bool > *clock_inif = o_avg_busy[0];
			sc_clock *clock_module = dynamic_cast<sc_clock*>(clock_inif);
			esc_hub_register_clock( clock_module, esc_alledge, 0, module_path, sim_domain, o_avg_busy.basename() );
			++registeredClocks;
